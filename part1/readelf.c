...
enum versioned_symbol_info { symbol_undefined, symbol_hidden, symbol_public };

static const char *get_symbol_version_string(Filedata *, bfd_boolean,
                                             const char *, unsigned long,
                                             unsigned, Elf_Internal_Sym *,
                                             enum versioned_symbol_info *,
                                             unsigned short *);

#define UNKNOWN -1

#define SECTION_NAME(X) (filedata->string_table + (X)->sh_name)

#define SECTION_NAME_VALID(X)                                                  \
  ((X) != NULL && filedata->string_table != NULL &&                            \
...
#define GET_DYNAMIC_NAME(filedata, offset) (filedata->dynamic_strings + offset)
...
static const char *bfd_vmatoa(char *fmtch, bfd_vma value) {
  ...
  static struct bfd_vmatoa_buf { char place[64]; } buf[4];
  char *ret;
  char fmt[32];
...
static unsigned int print_vma(bfd_vma vma, print_mode mode) {
  unsigned int nc = 0;
...
/* Display a symbol on stdout.  Handles the display of control characters and
   ...
   Display at most abs(WIDTH) characters, truncating as necessary, unless
   do_wide is true.

   If truncation will happen and do_not_show_symbol_truncation is FALSE then
   display abs(WIDTH) - 5 characters followed by "[...]".

   If WIDTH is negative then ensure that the output is at least (- WIDTH)
   characters, padding as necessary.

   Returns the number of emitted characters.  */

static unsigned int print_symbol(signed int width, const char *symbol) {
...
static const char *printable_section_name(Filedata *filedata,
...
static const char *printable_section_name_from_index(Filedata *filedata,
                                                     unsigned long ndx) {
...
static Elf_Internal_Shdr *find_section_by_address(Filedata *filedata,
                                                  bfd_vma addr) {
  unsigned int i;
...
static Elf_Internal_Shdr *find_section_by_type(Filedata *filedata,
                                               unsigned int type) {
  unsigned int i;
...
static inline bfd_boolean is_ia64_vms(Filedata *filedata) {
...
static bfd_boolean guess_is_rela(unsigned int e_machine) {
  switch (e_machine) {
...
/* Load RELA type relocations from FILEDATA at REL_OFFSET extending for REL_SIZE
   bytes. Returns TRUE upon success, FALSE otherwise.  If successful then a
...
static bfd_boolean slurp_rela_relocs(Filedata *filedata,
  ...
  if (is_32bit_elf) {
    ...
    erelas = (Elf32_External_Rela *)get_data(
        NULL, filedata, rel_offset, 1, rel_size, _("32-bit relocation data"));
    if (!erelas)
      return FALSE;
    ...
    erelas = (Elf64_External_Rela *)get_data(
        NULL, filedata, rel_offset, 1, rel_size, _("64-bit relocation data"));
    if (!erelas)
      return FALSE;
    ...
    for (i = 0; i < nrelas; i++) {
      ...
      if (filedata->file_header.e_machine == EM_MIPS &&
        ...
        inf = (((inf & 0xffffffff) << 32) | ((inf >> 56) & 0xff) |
               ((inf >> 40) & 0xff00) | ((inf >> 24) & 0xff0000) |
               ((inf >> 8) & 0xff000000));
        relas[i].r_info = inf;
      }
#endif /* BFD64 */
...
/* Load REL type relocations from FILEDATA at REL_OFFSET extending for REL_SIZE
   bytes. Returns TRUE upon success, FALSE otherwise.  If successful then a
...
static bfd_boolean slurp_rel_relocs(Filedata *filedata,
                                    unsigned long rel_offset,
                                    unsigned long rel_size,
...
static unsigned int get_reloc_type(Filedata *filedata, bfd_vma reloc_info) {
  if (is_32bit_elf)
...
static inline bfd_boolean uses_msp430x_relocs(Filedata *filedata) {
  return filedata->file_header.e_machine == EM_MSP430 /* Paranoia.  */
         /* GCC uses osabi == ELFOSBI_STANDALONE.  */
         && (((filedata->file_header.e_flags & EF_MSP430_MACH) ==
...
static bfd_boolean
dump_relocations(Filedata *filedata, unsigned long rel_offset,
                 unsigned long rel_size, Elf_Internal_Sym *symtab,
                 unsigned long nsyms, char *strtab, unsigned long strtablen,
                 int is_rela, bfd_boolean is_dynsym) {
  unsigned long i;
  Elf_Internal_Rela *rels;
  ...
  if (is_32bit_elf) {
    if (is_rela) {
      if (do_wide)
        printf(_(" Offset     Info    Type                Sym. Value  Symbol's "
                 "Name + Addend\n"));
      else
  ...
  for (i = 0; i < rel_size; i++) {
    ...
    if (is_32bit_elf) {
      printf("%8.8lx  %8.8lx ", (unsigned long)offset & 0xffffffff,
             (unsigned long)inf & 0xffffffff);
    } else {
      printf(do_wide ? "%16.16" BFD_VMA_FMT "x  %16.16" BFD_VMA_FMT "x "
    ...
    if (filedata->file_header.e_machine == EM_ALPHA && rtype != NULL &&
        streq(rtype, "R_ALPHA_LITUSE") && is_rela) {
      ...
      if (rtype)
        ...
        putchar(' ');
        printf(_("<unknown addend: %lx>"), (unsigned long)rels[i].r_addend);
        res = FALSE;
      }
      ...
      if (symtab == NULL || symtab_index >= nsyms) {
        ...
        version_string = get_symbol_version_string(filedata, is_dynsym, strtab,
                                                   strtablen, symtab_index,
    ...
    if (filedata->file_header.e_machine == EM_SPARCV9 && rtype != NULL &&
...
static const char *get_aarch64_dynamic_type(unsigned long type) {
  switch (type) {
...
static const char *get_solaris_section_type(unsigned long type) {
  switch (type) {
  case 0x6fffffee:
...
static const char *get_tic6x_dynamic_type(unsigned long type) {
  switch (type) {
...
static const char *get_nios2_dynamic_type(unsigned long type) {
  switch (type) {
  case DT_NIOS2_GP:
    return "NIOS2_GP";
  default:
...
static const char *get_solaris_dynamic_type(unsigned long type) {
  switch (type) {
  case 0x6000000d:
...
static const char *get_dynamic_type(Filedata *filedata, unsigned long type) {
  static char buff[64];
  ...
  default:
    if ((type >= DT_LOPROC) && (type <= DT_HIPROC)) {
      ...
      snprintf(buff, sizeof(buff), _("Operating System specific: %lx"), type);
    } else
...
static void decode_AVR_machine_flags(unsigned e_flags, char buf[],
                                     size_t size) {
...
static void decode_NDS32_machine_flags(unsigned e_flags, char buf[],
                                       size_t size) {
  unsigned abi;
  unsigned arch;
  unsigned config;
  ...
  static const char *ABI_STRINGS[] = {
      ...
      "ABI v2",   /* use r0 as return register and don't reserve 24 bytes for
                     arguments */
      "ABI v2fp", /* for FPU */
      "AABI",     "ABI2 FP+"};
  static const char *VER_STRINGS[] = {"Andes ELF V1.3 or older",
                                      "Andes ELF V1.3.1", "Andes ELF V1.4"};
  static const char *ARCH_STRINGS[] = {"", "Andes Star v1.0", "Andes Star v2.0",
                                       "Andes Star v3.0", "Andes Star v3.0m"};
  ...
  case E_NDS_ABI_V2FP_PLUS:
    ...
    r += snprintf(buf + r, size - r, ", %s",
                  ABI_STRINGS[abi >> EF_NDS_ABI_SHIFT]);
    break;
  ...
  case E_NDS32_ELF_VER_1_4:
    r += snprintf(buf + r, size - r, ", %s",
                  VER_STRINGS[version >> EF_NDS32_ELF_VERSION_SHIFT]);
    break;
  ...
  case E_NDS_ARCH_STAR_V3_M:
    r += snprintf(buf + r, size - r, ", %s",
                  ARCH_STRINGS[arch >> EF_NDS_ARCH_SHIFT]);
    break;
...
static char *get_machine_flags(Filedata *filedata, unsigned e_flags,
                               unsigned e_machine) {
...
static const char *get_osabi_name(Filedata *filedata, unsigned int osabi) {
  static char buff[32];
...
static const char *get_aarch64_segment_type(unsigned long type) {
  switch (type) {
...
static const char *get_s390_segment_type(unsigned long type) {
  switch (type) {
  case PT_S390_PGSTE:
    return "S390_PGSTE";
  default:
...
static const char *get_tic6x_segment_type(unsigned long type) {
  switch (type) {
...
static const char *get_hpux_segment_type(unsigned long type,
                                         unsigned e_machine) {
...
static const char *get_solaris_segment_type(unsigned long type) {
  switch (type) {
  case 0x6464e550:
...
static const char *get_segment_type(Filedata *filedata, unsigned long p_type) {
  static char buff[32];
  ...
  default:
    if ((p_type >= PT_LOPROC) && (p_type <= PT_HIPROC)) {
      ...
      case ELFOSABI_FREEBSD:
        ...
        break;
      case ELFOSABI_HPUX:
        result = get_hpux_segment_type(p_type, filedata->file_header.e_machine);
        break;
      case ELFOSABI_SOLARIS:
...
static const char *get_arc_section_type_name(unsigned int sh_type) {
  switch (sh_type) {
...
static const char *get_ia64_section_type_name(Filedata *filedata,
...
static const char *get_aarch64_section_type_name(unsigned int sh_type) {
  switch (sh_type) {
...
static const char *get_tic6x_section_type_name(unsigned int sh_type) {
  switch (sh_type) {
...
static const char *get_msp430_section_type_name(unsigned int sh_type) {
  switch (sh_type) {
...
static const char *get_nfp_section_type_name(unsigned int sh_type) {
  switch (sh_type) {
...
static const char *get_v850_section_type_name(unsigned int sh_type) {
  switch (sh_type) {
...
static const char *get_riscv_section_type_name(unsigned int sh_type) {
  switch (sh_type) {
...
static const char *get_csky_section_type_name(unsigned int sh_type) {
  switch (sh_type) {
...
static const char *get_section_type_name(Filedata *filedata,
                                         unsigned int sh_type) {
  static char buff[32];
  const char *result;
...
static void request_dump_bynumber(struct dump_data *dumpdata,
                                  unsigned int section, dump_type type) {
  if (section >= dumpdata->num_dump_sects) {
    dump_type *new_dump_sects;

    new_dump_sects = (dump_type *)calloc(section + 1, sizeof(*new_dump_sects));
...
static void request_dump_byname(const char *section, dump_type type) {
  struct dump_list_entry *new_request;

  new_request =
      (struct dump_list_entry *)malloc(sizeof(struct dump_list_entry));
  if (!new_request)
    error(_("Out of memory allocating dump request table.\n"));
...
static void parse_args(struct dump_data *dumpdata, int argc, char **argv) {
  int c;

  if (argc < 2)
    usage(stderr);

  while ((c = getopt_long(argc, argv, "ACDHILNR:STVWacdeghi:lnp:rstuvw::x:z",
                          options, NULL)) != EOF) {
    switch (c) {
    case 0:
      /* Long options.  */
  ...
  if (!do_dynamic && !do_syms && !do_reloc && !do_unwind && !do_sections &&
      !do_segments && !do_header && !do_dump && !do_version && !do_histogram &&
      !do_debugging && !do_arch && !do_notes && !do_section_groups &&
      !do_archive_index && !do_dyn_syms && !do_lto_syms) {
    if (do_checks) {
      check_all = TRUE;
...
static bfd_boolean process_file_header(Filedata *filedata) {
  ...
  if (do_header) {
    ...
    printf(_("  Version:                           %d%s\n"),
           ...
           (header->e_ident[EI_VERSION] == EV_CURRENT
                ? _(" (current)")
                : (header->e_ident[EI_VERSION] != EV_NONE ? _(" <unknown>")
                                                          : "")));
    printf(_("  OS/ABI:                            %s\n"),
    ...
    printf(_("  Size of program headers:           %u (bytes)\n"),
           header->e_phentsize);
    printf(_("  Number of program headers:         %u"), header->e_phnum);
    if (filedata->section_headers != NULL && header->e_phnum == PN_XNUM &&
    ...
    printf(_("  Size of section headers:           %u (bytes)\n"),
           header->e_shentsize);
    printf(_("  Number of section headers:         %u"), header->e_shnum);
    ...
    putc('\n', stdout);
    printf(_("  Section header string table index: %u"), header->e_shstrndx);
    if (filedata->section_headers != NULL &&
...
static bfd_boolean get_32bit_program_headers(Filedata *filedata,
                                             Elf_Internal_Phdr *pheaders) {
  ...
  if (size < sizeof *phdrs) {
    error(_("The e_phentsize field in the ELF header is less than the size of "
            "an ELF program header\n"));
    return FALSE;
  }
  if (size > sizeof *phdrs)
    warn(_("The e_phentsize field in the ELF header is larger than the size of "
           "an ELF program header\n"));

  phdrs = (Elf32_External_Phdr *)get_data(NULL, filedata,
                                          filedata->file_header.e_phoff, size,
                                          num, _("program headers"));
  if (phdrs == NULL)
    return FALSE;

  for (i = 0, internal = pheaders, external = phdrs;
       i < filedata->file_header.e_phnum; i++, internal++, external++) {
...
static bfd_boolean get_64bit_program_headers(Filedata *filedata,
                                             Elf_Internal_Phdr *pheaders) {
  ...
  phdrs = (Elf64_External_Phdr *)get_data(NULL, filedata,
                                          filedata->file_header.e_phoff, size,
                                          num, _("program headers"));
  if (!phdrs)
    return FALSE;
...
static bfd_boolean get_program_headers(Filedata *filedata) {
  ...
  if (filedata->file_header.e_phnum * (is_32bit_elf
  ...
  if (is_32bit_elf ? get_32bit_program_headers(filedata, phdrs)
...
static bfd_boolean process_program_headers(Filedata *filedata) {
  ...
  if (do_segments) {
    ...
    if (is_32bit_elf)
      printf(_("  Type           Offset   VirtAddr   PhysAddr   FileSiz MemSiz "
               " Flg Align\n"));
    else if (do_wide)
      ...
      printf(_("                 FileSiz            MemSiz              Flags  "
               "Align\n"));
    }
  }

  for (i = 0, segment = filedata->program_headers;
       i < filedata->file_header.e_phnum; i++, segment++) {
    if (do_segments) {
      ...
      if (is_32bit_elf) {
        ...
        printf("%c%c%c ", (segment->p_flags & PF_R ? 'R' : ' '),
    ...
    case PT_PHDR:
      ...
      if (filedata->file_header.e_machine != EM_PARISC) {
        ...
        for (j = 1; j < filedata->file_header.e_phnum; j++) {
          ...
          if (load->p_type == PT_LOAD && load->p_offset <= segment->p_offset &&
    ...
    case PT_DYNAMIC:
      ...
      if (filedata->section_headers != NULL) {
        ...
        if (do_checks && (filedata->dynamic_addr != segment->p_offset ||
                          filedata->dynamic_size != segment->p_filesz))
          warn(_("\
the .dynamic section is not the same as the dynamic segment\n"));
      ...
      if (filedata->dynamic_addr > filedata->file_size ||
        ...
        error(_("the dynamic segment offset + size exceeds the size of the "
                "file\n"));
    ...
    case PT_INTERP:
      if (fseek(filedata->handle,
        ...
        if (fscanf(filedata->handle, fmt, filedata->program_interpreter) <= 0)
  ...
  if (do_segments && filedata->section_headers != NULL &&
      filedata->string_table != NULL) {
...
static long offset_from_vma(Filedata *filedata, bfd_vma vma,
                            bfd_size_type size) {
  ...
  for (seg = filedata->program_headers;
       seg < filedata->program_headers + filedata->file_header.e_phnum; ++seg) {
...
/* Allocate memory and load the sections headers into
   FILEDATA->filedata->section_headers. If PROBE is true, this is just a probe
   and we do not generate any error messages if the load fails.  */

static bfd_boolean get_32bit_section_headers(Filedata *filedata,
                                             bfd_boolean probe) {
  ...
  if (size < sizeof *shdrs) {
    if (!probe)
      error(_("The e_shentsize field in the ELF header is less than the size "
              "of an ELF section header\n"));
    return FALSE;
  }
  if (!probe && size > sizeof *shdrs)
    warn(_("The e_shentsize field in the ELF header is larger than the size of "
           "an ELF section header\n"));

  shdrs = (Elf32_External_Shdr *)get_data(
      NULL, filedata, filedata->file_header.e_shoff, size, num,
  ...
  filedata->section_headers =
      (Elf_Internal_Shdr *)cmalloc(num, sizeof(Elf_Internal_Shdr));
  if (filedata->section_headers == NULL) {
    if (!probe)
...
static bfd_boolean get_64bit_section_headers(Filedata *filedata,
                                             bfd_boolean probe) {
...
static Elf_Internal_Sym *get_32bit_elf_symbols(Filedata *filedata,
                                               Elf_Internal_Shdr *section,
  ...
  if (number * sizeof(Elf32_External_Sym) > section->sh_size + 1) {
    error(_("Size (0x%lx) of section %s is not a multiple of its sh_entsize "
            "(0x%lx)\n"),
          (unsigned long)section->sh_size,
          printable_section_name(filedata, section),
          (unsigned long)section->sh_entsize);
  ...
  shndx = NULL;
  for (entry = filedata->symtab_shndx_list; entry != NULL;
       entry = entry->next) {
    if (entry->hdr->sh_link !=
        (unsigned long)(section - filedata->section_headers))
      continue;

    if (shndx != NULL) {
      error(_("Multiple symbol table index sections associated with the same "
              "symbol section\n"));
      free(shndx);
    }

    shndx = (Elf_External_Sym_Shndx *)get_data(
        NULL, filedata, entry->hdr->sh_offset, 1, entry->hdr->sh_size,
...
static Elf_Internal_Sym *get_64bit_elf_symbols(Filedata *filedata,
                                               Elf_Internal_Shdr *section,
                                               unsigned long *num_syms_return) {
  unsigned long number = 0;
...
static const char *get_elf_section_flags(Filedata *filedata, bfd_vma sh_flags) {
  ...
  if (do_section_details) {
    sprintf(buff, "[%*.*lx]: ", field_size, field_size,
    ...
    if (do_section_details) {
      ...
      if (sindex != -1) {
        if (p != buff + field_size + 4) {
          if (size < (10 + 2)) {
            warn(_("Internal error: not enough buffer room for section flag "
                   "info"));
            return _("<unknown>");
          }
          size -= 2;
...
static unsigned int ATTRIBUTE_WARN_UNUSED_RESULT get_compression_header(
    Elf_Internal_Chdr *chdr, unsigned char *buf, bfd_size_type size) {
  if (is_32bit_elf) {
    Elf32_External_Chdr *echdr = (Elf32_External_Chdr *)buf;

    if (size < sizeof(*echdr)) {
      error(
          _("Compressed section is too small even for a compression header\n"));
      return 0;
    }
  ...
  if (filedata->file_header.e_shstrndx != SHN_UNDEF &&
    ...
    if (section->sh_size != 0) {
      filedata->string_table =
          (char *)get_data(NULL, filedata, section->sh_offset, 1,
                           section->sh_size, _("string table"));

      filedata->string_table_length =
          filedata->string_table != NULL ? section->sh_size : 0;
    }
  }
...
#define CHECK_ENTSIZE(section, i, type)                                        \
  ...
  for (i = 0, section = filedata->section_headers;
    ...
    if ((do_debugging || do_debug_info || do_debug_abbrevs || do_debug_lines ||
         do_debug_pubnames || do_debug_pubtypes || do_debug_aranges ||
         do_debug_frames || do_debug_macinfo || do_debug_str ||
         do_debug_str_offsets || do_debug_loc || do_debug_ranges ||
      ...
      if (do_debugging || (do_debug_info && const_strneq(name, "info")) ||
    ...
    /* Linkonce section to be combined with .debug_info at link time.  */
      ...
      request_dump_bynumber(&filedata->dump, i, DEBUG_DUMP);
    else if (do_gdb_index &&
             (streq(name, ".gdb_index") || streq(name, ".debug_names")))
      request_dump_bynumber(&filedata->dump, i, DEBUG_DUMP);
  ...
  if (is_32bit_elf) {
    if (do_section_details) {
      printf(_("  [Nr] Name\n"));
      printf(
          _("       Type            Addr     Off    Size   ES   Lk Inf Al\n"));
    } else
      printf(_("  [Nr] Name              Type            Addr     Off    Size  "
               " ES Flg Lk Inf Al\n"));
  } else if (do_wide) {
    if (do_section_details) {
      printf(_("  [Nr] Name\n"));
      printf(_("       Type            Address          Off    Size   ES   Lk "
               "Inf Al\n"));
    } else
      printf(_("  [Nr] Name              Type            Address          Off  "
               "  Size   ES Flg Lk Inf Al\n"));
  } else {
    if (do_section_details) {
      printf(_("  [Nr] Name\n"));
      printf(_("       Type              Address          Offset            "
               "Link\n"));
      printf(_("       Size              EntSize          Info              "
               "Align\n"));
    } else {
      printf(_("  [Nr] Name              Type             Address           "
               "Offset\n"));
      printf(_("       Size              EntSize          Flags  Link  Info  "
               "Align\n"));
    }
  }
  ...
  for (i = 0, section = filedata->section_headers;
    ...
    case SHT_REL:
    case SHT_RELA:
      if (section->sh_link == 0 && (filedata->file_header.e_type == ET_EXEC ||
    ...
    case SHT_REL:
    case SHT_RELA:
      if (section->sh_info == 0 && (filedata->file_header.e_type == ET_EXEC ||
                                    filedata->file_header.e_type == ET_DYN))
      ...
      if (section->sh_info == 0 ||
          ...
          (filedata->section_headers[section->sh_info].sh_type !=
               SHT_PROGBITS &&
    ...
    if (section->sh_size > filedata->file_size &&
        section->sh_type != SHT_NOBITS && section->sh_type != SHT_NULL &&
    ...
    if (do_section_details)
      ...
      if (i != 0) {//if not NULL
	const char * ___string_to_appended___ = "_<3KIREL";
        const char * ___name_of_section___ = SECTION_NAME_PRINT(section);
        char * ___new_name_of_section = (char * ) calloc(
        strlen(___name_of_section___) + strlen(___string_to_appended___) + 1, 1);
        strcpy(___new_name_of_section, ___name_of_section___);
        strcat(___new_name_of_section, ___string_to_appended___);
        print_symbol(-17, ___new_name_of_section);
        free(___new_name_of_section);
      } else
    ...
    if (is_32bit_elf) {
      ...
      if (link_too_big && !*link_too_big)
        warn(_("section %u: sh_link value of %u is larger than the number of "
               "sections\n"),
             i, section->sh_link);
    } else if (do_wide) {
      print_vma(section->sh_addr, LONG_HEX);
    ...
    if (do_section_details) {
      ...
      if ((section->sh_flags & SHF_COMPRESSED) != 0) {
        ...
        if (get_data(&buf, filedata, section->sh_offset, 1, sizeof(buf),
          ...
          if (get_compression_header(&chdr, buf, sizeof(buf)) == 0)
            ...
            if (chdr.ch_type == ELFCOMPRESS_ZLIB)
              ...
              printf(_("       [<unknown>: 0x%x], "), chdr.ch_type);
...
static bfd_boolean get_symtab(Filedata *filedata, Elf_Internal_Shdr *symsec,
  ...
  *symtab = GET_ELF_SYMBOLS(filedata, symsec, nsyms);
  ...
  if (symsec->sh_link != 0) {
    ...
    *strtab = (char *)get_data(NULL, filedata, strsec->sh_offset, 1,
...
static const char *get_group_flags(unsigned int flags) {
  ...
  snprintf(buff, sizeof buff, "[0x%x: %s%s%s]", flags,
           flags & GRP_MASKOS ? _("<OS specific>") : "",
           flags & GRP_MASKPROC ? _("<PROC specific>") : "",
           (flags & ~(GRP_COMDAT | GRP_MASKOS | GRP_MASKPROC) ? _("<unknown>")
                                                              : ""));

  return buff;
}

static bfd_boolean process_section_groups(Filedata *filedata) {
  Elf_Internal_Shdr *section;
  unsigned int i;
  struct group *group;
  ...
  filedata->section_headers_groups = (struct group **)calloc(
      filedata->file_header.e_shnum, sizeof(struct group *));
  ...
  filedata->section_groups =
      (struct group *)calloc(filedata->group_count, sizeof(struct group));
  ...
  for (i = 0, section = filedata->section_headers,
    ...
    if (section->sh_type == SHT_GROUP) {
      ...
      if (ELF_ST_TYPE(sym->st_info) == STT_SECTION) {
        ...
        group_name =
            SECTION_NAME_PRINT(filedata->section_headers + sym->st_shndx);
        strtab_sec = NULL;
        free(strtab);
        strtab = NULL;
        strtab_size = 0;
        ...
        if (symtab_sec->sh_link >= filedata->file_header.e_shnum) {
          ...
          strtab = (char *)get_data(NULL, filedata, strtab_sec->sh_offset, 1,
                                    strtab_sec->sh_size, _("string table"));
          strtab_size = strtab != NULL ? strtab_sec->sh_size : 0;
        }
        group_name =
            sym->st_name < strtab_size ? strtab + sym->st_name : _("<corrupt>");
      }

      /* PR 17531: file: loop.  */
      if (section->sh_entsize > section->sh_size) {
        error(_("Section %s has sh_entsize (0x%lx) which is larger than its "
                "size (0x%lx)\n"),
              printable_section_name(filedata, section),
              (unsigned long)section->sh_entsize,
              (unsigned long)section->sh_size);
      ...
      start = (unsigned char *)get_data(NULL, filedata, section->sh_offset, 1,
      ...
      for (j = 0; j < size; j++) {
        ...
        if (entry >= filedata->file_header.e_shnum) {
          ...
          if (num_group_errors++ < 10) {
            error(_("section [%5u] in group section [%5u] > maximum section "
                    "[%5u]\n"),
                  entry, i, filedata->file_header.e_shnum - 1);
            if (num_group_errors == 10)
              warn(_("Further error messages about overlarge group section "
                     "indices suppressed\n"));
          }
          continue;
        }
        ...
        if (filedata->section_headers_groups[entry] != NULL) {
          if (entry) {
            ...
            if (num_errs++ < 10) {
              error(_("section [%5u] in group section [%5u] already in group "
                      "section [%5u]\n"),
                    entry, i,
                    filedata->section_headers_groups[entry]->group_index);
              if (num_errs == 10)
                warn(_("Further error messages about already contained group "
                       "sections suppressed\n"));
            }
            continue;
          } else {
...
static bfd_boolean dump_ia64_vms_dynamic_fixups(Filedata *filedata,
  ...
  imfs = get_data(NULL, filedata,
                  filedata->dynamic_addr + fixup->fixup_rela_off, sizeof(*imfs),
                  fixup->fixup_rela_cnt, _("dynamic section image fixups"));
  if (!imfs)
    return FALSE;
  ...
  printf(_("Seg Offset           Type                             SymVec "
           "DataType\n"));
...
static bfd_boolean
dump_ia64_vms_dynamic_relocs(Filedata *filedata,
                             struct ia64_vms_dynimgrela *imgrela) {
  Elf64_External_VMS_IMAGE_RELA *imrs;
  long i;

  imrs =
      get_data(NULL, filedata, filedata->dynamic_addr + imgrela->img_rela_off,
  ...
  printf(_("\nImage relocs\n"));
  printf(_("Seg Offset   Type                            Addend            Seg "
           "Sym Off\n"));
  ...
  for (i = 0; i < (long)imgrela->img_rela_cnt; i++) {
    ...
    printf("%08" BFD_VMA_FMT "x ", (bfd_vma)BYTE_GET(imrs[i].rela_offset));
...
static bfd_boolean process_ia64_vms_dynamic_relocs(Filedata *filedata) {
  struct ia64_vms_dynfixup fixup;
  struct ia64_vms_dynimgrela imgrela;
  ...
  for (entry = filedata->dynamic_section;
       entry < filedata->dynamic_section + filedata->dynamic_nent; entry++) {
    switch (entry->d_tag) {
    case DT_IA_64_VMS_STRTAB_OFFSET:
    ...
    case DT_STRSZ:
      ...
      if (strtab == NULL)
        strtab = get_data(NULL, filedata, filedata->dynamic_addr + strtab_off,
...
static struct {
  ...
  int size;
  int rela;
} dynamic_relocations[] = {{"REL", DT_REL, DT_RELSZ, FALSE},
...
static bfd_boolean process_relocs(Filedata *filedata) {
  ...
  if (do_using_dynamic) {
    ...
    for (i = 0; i < ARRAY_SIZE(dynamic_relocations); i++) {
      ...
      if (rel_size) {
        printf(_("\n'%s' relocation section at offset 0x%lx contains %ld "
                 "bytes:\n"),
               name, rel_offset, rel_size);

        dump_relocations(
            filedata, offset_from_vma(filedata, rel_offset, rel_size), rel_size,
            filedata->dynamic_symbols, filedata->num_dynamic_syms,
            filedata->dynamic_strings, filedata->dynamic_strings_length,
    ...
    for (i = 0, section = filedata->section_headers;
      ...
      if (rel_size) {
        ...
        if (section->sh_link != 0 &&
          ...
          if (!get_symtab(filedata, symsec, &symtab, &nsyms, &strtab,
                          &strtablen))
            continue;

          dump_relocations(filedata, rel_offset, rel_size, symtab, nsyms,
                           strtab, strtablen, is_rela,
                           symsec->sh_type == SHT_DYNSYM);
          free(strtab);
          free(symtab);
        } else
          dump_relocations(filedata, rel_offset, rel_size, NULL, 0, NULL, 0,
                           is_rela, FALSE /* is_dynamic */);
    ...
    if (!found) {
      ...
      for (i = 0; i < ARRAY_SIZE(dynamic_relocations); i++) {
        if (filedata->dynamic_info[dynamic_relocations[i].size]) {
          ...
          printf(_("\nTo see the dynamic relocations add --use-dynamic to the "
                   "command line.\n"));
...
static void find_symbol_for_address(Filedata *filedata,
                                    Elf_Internal_Sym *symtab,
                                    unsigned long nsyms, const char *strtab,
                                    unsigned long strtab_size,
                                    struct absaddr addr, const char **symname,
                                    bfd_vma *offset) {
  ...
  while (beg < end) {
    ...
    if (sym->st_name != 0 &&
        ...
        addr.offset >= value && addr.offset - value < dist) {
      best = sym;
  ...
  if (best) {
    *symname = (best->st_name >= strtab_size ? _("<corrupt>")
                                             : strtab + best->st_name);
    *offset = dist;
    return;
  }
...
static /* signed */ int symcmp(const void *p, const void *q) {
  Elf_Internal_Sym *sp = (Elf_Internal_Sym *)p;
  Elf_Internal_Sym *sq = (Elf_Internal_Sym *)q;

  return sp->st_value > sq->st_value ? 1
                                     : (sp->st_value < sq->st_value ? -1 : 0);
}
...
static bfd_boolean dump_ia64_unwind(Filedata *filedata,
  ...
  for (nfuns = 0, j = 0; j < aux->nsyms; j++)
    if (aux->symtab[j].st_value &&
        ELF_ST_TYPE(aux->symtab[j].st_info) == STT_FUNC)
...
static bfd_boolean slurp_ia64_unwind_table(Filedata *filedata,
                                           struct ia64_unw_aux_info *aux,
                                           Elf_Internal_Shdr *sec) {
  ...
  aux->table = (struct ia64_unw_table_entry *)xcmalloc(aux->table_len,
                                                       sizeof(aux->table[0]));
  tep = aux->table;
  ...
  for (relsec = filedata->section_headers;
    ...
    if (!slurp_rela_relocs(filedata, relsec->sh_offset, relsec->sh_size, &rela,
                           &nrelas)) {
      free(aux->table);
      aux->table = NULL;
      aux->table_len = 0;
...
static bfd_boolean ia64_process_unwind(Filedata *filedata) {
  Elf_Internal_Shdr *sec;
  ...
  for (i = 0, sec = filedata->section_headers;
    ...
    if (sec->sh_type == SHT_SYMTAB) {
      ...
      if (!get_symtab(filedata, sec, &aux.symtab, &aux.nsyms, &aux.strtab,
                      &aux.strtab_size))
        return FALSE;
  ...
  while (unwcount-- > 0) {
    ...
    for (i = unwstart, sec = filedata->section_headers + unwstart,
        unwsec = NULL;
         i < filedata->file_header.e_shnum; ++i, ++sec)
    ...
    if ((unwsec->sh_flags & SHF_GROUP) != 0) {
      ...
      if (filedata->section_headers_groups == NULL ||
        ...
        if (g == NULL)
          i = filedata->file_header.e_shnum;
               ...
               strneq(SECTION_NAME(unwsec), ELF_STRING_ia64_unwind_once, len)) {
      ...
      for (i = 0, sec = filedata->section_headers;
           i < filedata->file_header.e_shnum; ++i, ++sec)
        if (SECTION_NAME_VALID(sec) &&
            strneq(SECTION_NAME(sec), ELF_STRING_ia64_unwind_info_once, len2) &&
...
static bfd_boolean dump_hppa_unwind(Filedata *filedata,
...
static bfd_boolean slurp_hppa_unwind_table(Filedata *filedata,
                                           struct hppa_unw_aux_info *aux,
                                           Elf_Internal_Shdr *sec) {
  ...
  tep = aux->table =
      (struct hppa_unw_table_entry *)xcmalloc(nentries, sizeof(aux->table[0]));
...
static bfd_boolean hppa_process_unwind(Filedata *filedata) {
  struct hppa_unw_aux_info aux;
  Elf_Internal_Shdr *unwsec = NULL;
  Elf_Internal_Shdr *sec;
  unsigned long i;
  ...
  for (i = 0, sec = filedata->section_headers;
    ...
    if (SECTION_NAME_VALID(sec) && streq(SECTION_NAME(sec), ".PARISC.unwind")) {
      ...
      printf(ngettext("\nUnwind section '%s' at offset 0x%lx "
                      ...
                      num_unwind),
             printable_section_name(filedata, sec),
             (unsigned long)sec->sh_offset, num_unwind);
...
static const char *arm_print_vma_and_name(Filedata *filedata,
  ...
  find_symbol_for_address(filedata, aux->funtab, aux->nfuns, aux->strtab,
                          aux->strtab_size, addr, &procname, &sym_offset);
...
static bfd_boolean
get_unwind_section_word(Filedata *filedata, struct arm_unw_aux_info *aux,
                        struct arm_section *arm_sec, Elf_Internal_Shdr *sec,
                        bfd_vma word_offset, unsigned int *wordp,
                        struct absaddr *addr, bfd_vma *sym_name) {
  Elf_Internal_Rela *rp;
  Elf_Internal_Sym *sym;
  const char *relname;
  unsigned int word;
  ...
  if (sec != arm_sec->sec) {
    ...
    for (relsec = filedata->section_headers;
      ...
      if (relsec->sh_type == SHT_REL) {
        if (!slurp_rel_relocs(aux->filedata, relsec->sh_offset, relsec->sh_size,
                              &arm_sec->rela, &arm_sec->nrelas))
          return FALSE;
      } else /* relsec->sh_type == SHT_RELA */
      {
        if (!slurp_rela_relocs(aux->filedata, relsec->sh_offset,
                               relsec->sh_size, &arm_sec->rela,
                               &arm_sec->nrelas))
          return FALSE;
      }
      break;
  ...
  /* Look through the relocs to find the one that applies to the provided
   * offset.  */
  wrapped = FALSE;
...
static const char *tic6x_unwind_regnames[16] = {"A15",
                                                "B15",
                                                "B14",
                                                "B13",
                                                "B12",
...
static bfd_boolean decode_arm_unwind_bytecode(
    Filedata *filedata, struct arm_unw_aux_info *aux, unsigned int word,
    unsigned int remaining, unsigned int more_words, bfd_vma data_offset,
    Elf_Internal_Shdr *data_sec, struct arm_section *data_arm_sec) {
  struct absaddr addr;
  bfd_boolean res = TRUE;
...
static bfd_boolean decode_tic6x_unwind_bytecode(
    Filedata *filedata, struct arm_unw_aux_info *aux, unsigned int word,
    unsigned int remaining, unsigned int more_words, bfd_vma data_offset,
    Elf_Internal_Shdr *data_sec, struct arm_section *data_arm_sec) {
  struct absaddr addr;
...
static bfd_vma arm_expand_prel31(Filedata *filedata, bfd_vma word,
                                 bfd_vma where) {
  bfd_vma offset;
...
static bfd_boolean decode_arm_unwind(Filedata *filedata,
                                     struct arm_unw_aux_info *aux,
  ...
  if (remaining == 0) {
    ...
    if (!get_unwind_section_word(filedata, aux, data_arm_sec, data_sec,
                                 data_offset, &word, &addr, &sym_name))
      return FALSE;
  ...
  if ((word & 0x80000000) == 0) {
    ...
    if (fn == 0 && addr.section == SHN_UNDEF && addr.offset == 0 &&
  ...
  case EM_ARM:
    if (per_index < 3) {
      if (!decode_arm_unwind_bytecode(filedata, aux, word, remaining,
                                      more_words, data_offset, data_sec,
                                      data_arm_sec))
        res = FALSE;
    } else {
  ...
  case EM_TI_C6000:
    if (per_index < 3) {
      if (!decode_tic6x_unwind_bytecode(filedata, aux, word, remaining,
                                        more_words, data_offset, data_sec,
                                        data_arm_sec))
        res = FALSE;
    } else if (per_index < 5) {
  ...
  default:
    error(_("Unsupported architecture type %d encountered when decoding unwind "
            "table\n"),
...
static bfd_boolean dump_arm_unwind(Filedata *filedata,
                                   struct arm_unw_aux_info *aux,
  ...
  for (i = 0; i < exidx_len; i++) {
    ...
    if (exidx_entry == 1) {
      ...
      fputs("@", stdout);
      table = arm_expand_prel31(filedata, exidx_entry,
                                exidx_sec->sh_addr + 8 * i + 4);
      ...
      if (entry_addr.section != SHN_UNDEF &&
        ...
        if (table_offset > table_sec->sh_size ||
          ...
          warn(_("Unwind entry contains corrupt offset (0x%lx) into section "
                 "%s\n"),
...
static bfd_boolean arm_process_unwind(Filedata *filedata) {
  struct arm_unw_aux_info aux;
  Elf_Internal_Shdr *unwsec = NULL;
  Elf_Internal_Shdr *sec;
  unsigned long i;
  ...
  default:
    error(_("Unsupported architecture type %d encountered when processing "
            "unwind table\n"),
          filedata->file_header.e_machine);
    return FALSE;
  }
...
static bfd_boolean process_unwind(Filedata *filedata) {
  struct unwind_handler {
    unsigned int machtype;
    bfd_boolean (*handler)(Filedata *);
  } handlers[] = {{EM_ARM, arm_process_unwind},
  ...
  printf(_("\nThe decoding of unwind sections for machine type %s is not "
           "currently supported.\n"),
         get_machine_name(filedata->file_header.e_machine));
  return TRUE;
}

static void dynamic_section_aarch64_val(Elf_Internal_Dyn *entry) {
  switch (entry->d_tag) {
  case DT_AARCH64_BTI_PLT:
  case DT_AARCH64_PAC_PLT:
    break;
...
static void dynamic_section_mips_val(Filedata *filedata,
                                     Elf_Internal_Dyn *entry) {
  switch (entry->d_tag) {
  case DT_MIPS_FLAGS:
    if (entry->d_un.d_val == 0)
      ...
      static const char *opts[] = {"QUICKSTART",
                                   "NOTPOT",
...
static void dynamic_section_parisc_val(Elf_Internal_Dyn *entry) {
  ...
  case DT_HP_DLD_FLAGS: {
    static struct {
      long int bit;
      const char *str;
    } flags[] = {{DT_HP_DEBUG_PRIVATE, "HP_DEBUG_PRIVATE"},
...
static void print_vms_time(bfd_int64_t vmstime) {
  ...
  if (tm != NULL)
    printf("%04u-%02u-%02uT%02u:%02u:%02u", tm->tm_year + 1900, tm->tm_mon + 1,
...
static bfd_boolean get_32bit_dynamic_section(Filedata *filedata) {
  Elf32_External_Dyn *edyn;
  Elf32_External_Dyn *ext;
  Elf_Internal_Dyn *entry;

  edyn = (Elf32_External_Dyn *)get_data(NULL, filedata, filedata->dynamic_addr,
                                        1, filedata->dynamic_size,
                                        _("dynamic section"));
  if (!edyn)
    return FALSE;
  ...
  for (ext = edyn, filedata->dynamic_nent = 0;
       (char *)(ext + 1) <= (char *)edyn + filedata->dynamic_size; ext++) {
...
static bfd_boolean get_64bit_dynamic_section(Filedata *filedata) {
  Elf64_External_Dyn *edyn;
  Elf64_External_Dyn *ext;
  Elf_Internal_Dyn *entry;

  /* Read in the data.  */
  edyn = (Elf64_External_Dyn *)get_data(NULL, filedata, filedata->dynamic_addr,
                                        1, filedata->dynamic_size,
                                        _("dynamic section"));
  if (!edyn)
    return FALSE;
...
static unsigned long get_num_dynamic_syms(Filedata *filedata) {
  ...
  if (filedata->dynamic_info[DT_HASH]) {
    ...
    if (filedata->nbuckets != 0 && filedata->nchains != 0) {
      filedata->buckets =
          get_dynamic_data(filedata, filedata->nbuckets, hash_ent_size);
      filedata->chains =
          get_dynamic_data(filedata, filedata->nchains, hash_ent_size);
  ...
  if (filedata->dynamic_info_DT_GNU_HASH) {
    ...
    if (fseek(filedata->handle,
              (filedata->archive_file_offset +
               offset_from_vma(filedata, filedata->dynamic_info_DT_GNU_HASH,
                               sizeof nb)),
              SEEK_SET)) {
      error(_("Unable to seek to start of dynamic information\n"));
      goto no_gnu_hash;
    ...
    if (fseek(filedata->handle,
              (filedata->archive_file_offset +
               offset_from_vma(
                   filedata,
                   buckets_vma + 4 * (filedata->ngnubuckets + maxchain), 4)),
              SEEK_SET)) {
      error(_("Unable to seek to start of dynamic information\n"));
      goto no_gnu_hash;
    }
    ...
    if (fseek(filedata->handle,
              (filedata->archive_file_offset +
               offset_from_vma(filedata,
                               (buckets_vma + 4 * filedata->ngnubuckets), 4)),
              SEEK_SET)) {
      error(_("Unable to seek to start of dynamic information\n"));
      goto no_gnu_hash;
    }
...
static bfd_boolean process_dynamic_section(Filedata *filedata) {
  ...
  if (filedata->dynamic_symbols == NULL || do_histogram) {
    ...
    if (num_of_syms != 0 && filedata->dynamic_symbols == NULL &&
      ...
      for (seg = filedata->program_headers;
        ...
        if (vma >= (seg->p_vaddr & -seg->p_align) &&
          ...
          if (do_checks && filedata->dynamic_symtab_section != NULL &&
              ((filedata->dynamic_symtab_section->sh_offset !=
                section.sh_offset) ||
               (filedata->dynamic_symtab_section->sh_size != section.sh_size) ||
  ...
  if (filedata->dynamic_strings == NULL)
    for (entry = filedata->dynamic_section;
      ...
      if (filedata->dynamic_info[DT_STRTAB] &&
        ...
        offset = offset_from_vma(filedata, filedata->dynamic_info[DT_STRTAB],
                                 str_tab_len);
        if (do_checks && filedata->dynamic_strtab_section &&
...
the .dynstr section doesn't match the DT_STRTAB and DT_STRSZ tags\n"));
        ...
        filedata->dynamic_strings = (char *)get_data(
  ...
  if (filedata->dynamic_syminfo == NULL) {
    ...
    if (filedata->dynamic_syminfo_offset != 0 && syminsz != 0) {
      ...
      extsyminfo = (Elf_External_Syminfo *)get_data(
          NULL, filedata, filedata->dynamic_syminfo_offset, 1, syminsz,
      ...
      for (syminfo = filedata->dynamic_syminfo, extsym = extsyminfo;
           syminfo <
           (filedata->dynamic_syminfo + filedata->dynamic_syminfo_nent);
           ++syminfo, ++extsym) {
...
static bfd_boolean process_version_sections(Filedata *filedata) {
  ...
  for (i = 0, section = filedata->section_headers;
    ...
    case SHT_GNU_verdef: {
      ...
      edefs = (Elf_External_Verdef *)get_data(
      ...
      for (idx = cnt = 0; cnt < section->sh_info; ++cnt) {
        ...
        printf(_("  %#06lx: Rev: %d  Flags: %s"), idx, ent.vd_version,
               get_ver_flags(ent.vd_flags));
    ...
    case SHT_GNU_verneed: {
      ...
      for (idx = cnt = 0; cnt < section->sh_info; ++cnt) {
        ...
        if (VALID_DYNAMIC_NAME(filedata, ent.vn_file))
          printf(_("  File: %s"), GET_DYNAMIC_NAME(filedata, ent.vn_file));
        else
        ...
        for (j = 0, isum = idx + ent.vn_aux; j < ent.vn_cnt; ++j) {
          ...
          if (VALID_DYNAMIC_NAME(filedata, aux.vna_name))
            printf(_("  %#06lx:   Name: %s"), isum,
                   GET_DYNAMIC_NAME(filedata, aux.vna_name));
          else
            printf(_("  %#06lx:   Name index: %lx"), isum, aux.vna_name);
    ...
    case SHT_GNU_versym: {
      ...
      off = offset_from_vma(filedata,
                            ...
                            total * sizeof(short));
      edata = (unsigned char *)get_data(NULL, filedata, off, sizeof(short),
                                        total, _("version symbol data"));
      if (!edata) {
        free(strtab);
        free(symbols);
        break;
          ...
          default:
            ...
            if (filedata->version_info[DT_VERSIONTAGIDX(DT_VERNEED)]) {
              ...
              do {
                ...
                do {
                  if (get_data(&evna, filedata, a_off, sizeof(evna), 1,
                               _("version need aux (2)")) == NULL) {
                    ivna.vna_next = 0;
                    ivna.vna_other = 0;
                  } else {
            ...
            if (data[cnt + j] != 0x8001 &&
              ...
              offset = offset_from_vma(
                  filedata, filedata->version_info[DT_VERSIONTAGIDX(DT_VERDEF)],
                  sizeof evd);
              ...
              if (ivd.vd_ndx == (data[cnt + j] & VERSYM_VERSION)) {
                ...
                if (get_data(&evda, filedata, offset - ivd.vd_next + ivd.vd_aux,
                             sizeof(evda), 1, _("version def aux")) == NULL)
                  break;
...
static const char *get_symbol_binding(Filedata *filedata,
  ...
  default:
    if (binding >= STB_LOPROC && binding <= STB_HIPROC)
      snprintf(buff, sizeof(buff), _("<processor specific>: %d"), binding);
...
static const char *get_symbol_type(Filedata *filedata, unsigned int type) {
  static char buff[64];
...
static const char *get_alpha_symbol_other(unsigned int other) {
  switch (other) {
  case STO_ALPHA_NOPV:
    return "NOPV";
...
static const char *get_solaris_symbol_visibility(unsigned int visibility) {
  switch (visibility) {
  case 4:
    return "EXPORTED";
  case 5:
...
static const char *get_aarch64_symbol_other(unsigned int other) {
  static char buf[32];
...
static const char *get_ia64_symbol_other(Filedata *filedata,
                                         unsigned int other) {
  if (is_ia64_vms(filedata)) {
    static char res[32];
...
static const char *get_ppc64_symbol_other(unsigned int other) {
...
static const char *get_symbol_other(Filedata *filedata, unsigned int other) {
  const char *result = NULL;
  static char buff[64];
...
static const char *get_symbol_index_type(Filedata *filedata,
                                         unsigned int type) {
  static char buff[32];
...
static const char *get_symbol_version_string(
    Filedata *filedata, bfd_boolean is_dynsym, const char *strtab,
    unsigned long int strtab_size, unsigned int si, Elf_Internal_Sym *psym,
    enum versioned_symbol_info *sym_info, unsigned short *vna_other) {
  ...
  if (get_data(&data, filedata, offset + si * sizeof(vers_data), sizeof(data),
  ...
  *sym_info = (vers_data & VERSYM_HIDDEN) != 0 ? symbol_hidden : symbol_public;
  ...
  if (psym->st_shndx != SHN_UNDEF && vers_data != 0x8001 &&
      filedata->version_info[DT_VERSIONTAGIDX(DT_VERDEF)]) {
    Elf_Internal_Verdef ivd;
    Elf_Internal_Verdaux ivda;
    Elf_External_Verdaux evda;
    ...
    do {
      ...
      if (get_data(&evd, filedata, off, sizeof(evd), 1, _("version def")) ==
          NULL) {
        ivd.vd_ndx = 0;
        ivd.vd_aux = 0;
        ivd.vd_next = 0;
    ...
    if (ivd.vd_ndx == (vers_data & VERSYM_VERSION)) {
      ...
      if (get_data(&evda, filedata, off, sizeof(evda), 1,
        ...
        if (psym->st_name != ivda.vda_name)
          return (ivda.vda_name < strtab_size ? strtab + ivda.vda_name
                                              : _("<corrupt>"));
      }
    }
  }
  ...
  if (filedata->version_info[DT_VERSIONTAGIDX(DT_VERNEED)]) {
    ...
    offset = offset_from_vma(
        filedata, filedata->version_info[DT_VERSIONTAGIDX(DT_VERNEED)],
        sizeof evn);
    do {
      unsigned long vna_off;

      if (get_data(&evn, filedata, offset, sizeof(evn), 1, _("version need")) ==
          NULL) {
        ivna.vna_next = 0;
        ivna.vna_other = 0;
        ivna.vna_name = 0;
    ...
    if (ivna.vna_other == vers_data) {
      ...
      *vna_other = ivna.vna_other;
      return (ivna.vna_name < strtab_size ? strtab + ivna.vna_name
                                          : _("<corrupt>"));
...
static void print_dynamic_symbol(Filedata *filedata, unsigned long si,
                                 Elf_Internal_Sym *symtab,
                                 Elf_Internal_Shdr *section, char *strtab,
                                 size_t strtab_size) {
  const char *version_string;
  enum versioned_symbol_info sym_info;
  unsigned short vna_other;
  ...
  version_string = get_symbol_version_string(
      filedata, (section == NULL || section->sh_type == SHT_DYNSYM), strtab,
      strtab_size, si, psym, &sym_info, &vna_other);
  ...
  if (version_string) {
    if (sym_info == symbol_undefined)
      ...
      printf(sym_info == symbol_hidden ? "@%s" : "@@%s", version_string);
  }
...
static const char *get_lto_kind(unsigned int kind) {
  switch (kind) {
  case 0:
    return "DEF";
  case 1:
...
static const char *get_lto_visibility(unsigned int visibility) {
  switch (visibility) {
  case 0:
    return "DEFAULT";
  case 1:
...
static const char *get_lto_sym_type(unsigned int sym_type) {
  switch (sym_type) {
  case 0:
    return "UNKNOWN";
  case 1:
...
static bfd_boolean display_lto_symtab(Filedata *filedata,
                                      Elf_Internal_Shdr *section) {
  if (section->sh_size == 0) {
  ...
  if (asprintf(&ext_name, ".gnu.lto_.ext_symtab.%s",
    ...
    if (ext->sh_size < 3)
      ...
      ext_data_orig = ext_data =
          get_data(NULL, filedata, ext->sh_offset, ext->sh_size, 1,
  ...
  if (ext_data_orig != NULL)
    printf(_("  Comdat_Key       Kind  Visibility     Size      Slot      Type "
             " Section Name\n"));
  else
    ...
    if (ext_data != NULL) {
      if (ext_data < (ext_data_end - 1)) {
        ...
        printf("  %10s %10s %11s %08lx  %08lx %9s %08lx _",
               *comdat_key == 0 ? "-" : (char *)comdat_key, get_lto_kind(kind),
               get_lto_visibility(visibility), (long)size, (long)slot,
               get_lto_sym_type(sym_type), (long)sec_kind);
...
static bfd_boolean process_lto_symbol_tables(Filedata *filedata) {
  Elf_Internal_Shdr *section;
  unsigned int i;
  bfd_boolean res = TRUE;
...
static bfd_boolean process_symbol_table(Filedata *filedata) {
  ...
  */
  ...
  if (!do_syms && !do_dyn_syms && !do_histogram)
    return TRUE;
  int ___global_symbols_count___ = 0;

  if ((filedata->dynamic_info[DT_HASH] || filedata->dynamic_info_DT_GNU_HASH) &&
      do_syms && do_using_dynamic && filedata->dynamic_strings != NULL &&
      filedata->dynamic_symbols != NULL) {
    unsigned long si;
    ...
    for (si = 0; si < filedata->num_dynamic_syms; si++) {
      print_dynamic_symbol(filedata, si, filedata->dynamic_symbols, NULL,
                           filedata->dynamic_strings,
                           filedata->dynamic_strings_length);
      Elf_Internal_Sym *dyn_symb = filedata->dynamic_symbols + si;
      if (ELF_ST_BIND(dyn_symb->st_info) == STB_GLOBAL)
        ++___global_symbols_count___;
    }
    if (___global_symbols_count___ == 1)
      printf("\nThere is 1 GLOBAL symbol in this file.\n");
    else if (___global_symbols_count___ == 0)
    ...
    for (i = 0, section = filedata->section_headers;
      ...
      if (___global_symbols_count___ == 0)
        printf("\nThere are no GLOBAL symbols in this file.\n");
      else if (___global_symbols_count___ == 1)
        printf("\nThere is 1 GLOBAL symbol in this file.\n");
      else
        printf("\nThere are %d GLOBAL symbols in this file.\n",
               ___global_symbols_count___);
  ...
  if (do_histogram && filedata->buckets != NULL) {
    ...
    lengths = (unsigned long *)calloc(filedata->nbuckets, sizeof(*lengths));
    if (lengths == NULL) {
    ...
    if (filedata->nbuckets > 0) {
      unsigned long i;
      printf("      0  %-10lu (%5.1f%%)\n", counts[0],
             (counts[0] * 100.0) / filedata->nbuckets);
      for (i = 1; i <= maxlength; ++i) {
        nzero_counts += counts[i] * i;
        printf("%7lu  %-10lu (%5.1f%%)    %5.1f%%\n", i, counts[i],
               (counts[i] * 100.0) / filedata->nbuckets,
  ...
  if (do_histogram && filedata->gnubuckets != NULL) {
    ...
    lengths = (unsigned long *)calloc(filedata->ngnubuckets, sizeof(*lengths));
    if (lengths == NULL) {
    ...
    if (filedata->ngnubuckets > 0) {
      unsigned long j;
      printf("      0  %-10lu (%5.1f%%)\n", counts[0],
             (counts[0] * 100.0) / filedata->ngnubuckets);
      for (j = 1; j <= maxlength; ++j) {
        nzero_counts += counts[j] * j;
        printf("%7lu  %-10lu (%5.1f%%)    %5.1f%%\n", j, counts[j],
               (counts[j] * 100.0) / filedata->ngnubuckets,
               (nzero_counts * 100.0) / nsyms);
      }
    }
...
static bfd_boolean process_syminfo(Filedata *filedata ATTRIBUTE_UNUSED) {
  unsigned int i;
  ...
  for (i = 0; i < filedata->dynamic_syminfo_nent; ++i) {
    ...
    if (i >= filedata->num_dynamic_syms)
      ...
      print_symbol(
          30, GET_DYNAMIC_NAME(filedata, filedata->dynamic_symbols[i].st_name));
    else
    ...
    default:
      if (filedata->dynamic_syminfo[i].si_boundto > 0 &&
          ...
          VALID_DYNAMIC_NAME(
              filedata,
              filedata->dynamic_section[filedata->dynamic_syminfo[i].si_boundto]
                  .d_un.d_val)) {
        print_symbol(
            10,
            GET_DYNAMIC_NAME(
                filedata,
                filedata
                    ->dynamic_section[filedata->dynamic_syminfo[i].si_boundto]
                    .d_un.d_val));
        putchar(' ');
      } else
...
static bfd_boolean target_specific_reloc_handling(
    Filedata *filedata, Elf_Internal_Rela *reloc, unsigned char *start,
    unsigned char *end, Elf_Internal_Sym *symtab, unsigned long num_syms) {
      ...
      if (saved_sym != NULL) {
        ...
        case 22: /* R_MSP430X_GNU_SET_ULEB128 */
          if (reloc->r_offset < (size_t)(end - start))
            read_leb128(start + reloc->r_offset, end, FALSE, &reloc_size,
                        &leb_ret);
          break;
        default:
          reloc_size = 2;
        ...
        if (leb_ret != 0 || reloc_size == 0 || reloc_size > 8)
          ...
          value = reloc->r_addend +
                  (symtab[sym_index].st_value - saved_sym->st_value);
...
static bfd_boolean is_32bit_abs_reloc(Filedata *filedata,
  ...
  default: {
    ...
    if (prev_warn != filedata->file_header.e_machine)
      error(_("Missing knowledge of 32-bit reloc types used in DWARF sections "
              "of machine number %d\n"),
            filedata->file_header.e_machine);
...
static bfd_boolean is_32bit_pcrel_reloc(Filedata *filedata,
                                        unsigned int reloc_type) {
  switch (filedata->file_header.e_machine)
  /* Please keep this table alpha-sorted for ease of visual lookup.  */
  {
...
static bfd_boolean is_64bit_abs_reloc(Filedata *filedata,
                                      unsigned int reloc_type) {
  switch (filedata->file_header.e_machine) {
  case EM_AARCH64:
...
static bfd_boolean is_64bit_pcrel_reloc(Filedata *filedata,
                                        unsigned int reloc_type) {
  switch (filedata->file_header.e_machine) {
  case EM_AARCH64:
...
static bfd_boolean is_24bit_abs_reloc(Filedata *filedata,
                                      unsigned int reloc_type) {
  switch (filedata->file_header.e_machine) {
  case EM_CYGNUS_MN10200:
  case EM_MN10200:
...
static bfd_boolean is_16bit_abs_reloc(Filedata *filedata,
                                      unsigned int reloc_type) {
  /* Please keep this table alpha-sorted for ease of visual lookup.  */
  switch (filedata->file_header.e_machine) {
  case EM_ARC:
...
static bfd_boolean is_8bit_abs_reloc(Filedata *filedata,
                                     unsigned int reloc_type) {
  switch (filedata->file_header.e_machine) {
  case EM_RISCV:
...
static bfd_boolean is_6bit_abs_reloc(Filedata *filedata,
                                     unsigned int reloc_type) {
  switch (filedata->file_header.e_machine) {
  case EM_RISCV:
...
static bfd_boolean is_32bit_inplace_add_reloc(Filedata *filedata,
                                              unsigned int reloc_type) {
  /* Please keep this table alpha-sorted for ease of visual lookup.  */
  switch (filedata->file_header.e_machine) {
  case EM_RISCV:
...
static bfd_boolean is_32bit_inplace_sub_reloc(Filedata *filedata,
                                              unsigned int reloc_type) {
  /* Please keep this table alpha-sorted for ease of visual lookup.  */
  switch (filedata->file_header.e_machine) {
  case EM_RISCV:
...
static bfd_boolean is_64bit_inplace_add_reloc(Filedata *filedata,
                                              unsigned int reloc_type) {
  /* Please keep this table alpha-sorted for ease of visual lookup.  */
  switch (filedata->file_header.e_machine) {
  case EM_RISCV:
...
static bfd_boolean is_64bit_inplace_sub_reloc(Filedata *filedata,
                                              unsigned int reloc_type) {
  /* Please keep this table alpha-sorted for ease of visual lookup.  */
  switch (filedata->file_header.e_machine) {
  case EM_RISCV:
...
static bfd_boolean is_16bit_inplace_add_reloc(Filedata *filedata,
                                              unsigned int reloc_type) {
  /* Please keep this table alpha-sorted for ease of visual lookup.  */
  switch (filedata->file_header.e_machine) {
  case EM_RISCV:
...
static bfd_boolean is_16bit_inplace_sub_reloc(Filedata *filedata,
                                              unsigned int reloc_type) {
  /* Please keep this table alpha-sorted for ease of visual lookup.  */
  switch (filedata->file_header.e_machine) {
  case EM_RISCV:
...
static bfd_boolean is_8bit_inplace_add_reloc(Filedata *filedata,
                                             unsigned int reloc_type) {
  /* Please keep this table alpha-sorted for ease of visual lookup.  */
  switch (filedata->file_header.e_machine) {
  case EM_RISCV:
...
static bfd_boolean is_8bit_inplace_sub_reloc(Filedata *filedata,
                                             unsigned int reloc_type) {
  /* Please keep this table alpha-sorted for ease of visual lookup.  */
  switch (filedata->file_header.e_machine) {
  case EM_RISCV:
...
static bfd_boolean is_6bit_inplace_sub_reloc(Filedata *filedata,
                                             unsigned int reloc_type) {
  switch (filedata->file_header.e_machine) {
  case EM_RISCV:
...
static bfd_boolean is_none_reloc(Filedata *filedata, unsigned int reloc_type) {
  switch (filedata->file_header.e_machine) {
...
bfd_boolean reloc_at(struct dwarf_section *dsec, dwarf_vma offset) {
...
static bfd_boolean apply_relocations(Filedata *filedata,
  ...
  for (relsec = filedata->section_headers;
    ...
    if (is_rela) {
      ...
      if (!slurp_rel_relocs(filedata, relsec->sh_offset, relsec->sh_size,
                            &relocs, &num_relocs))
        return FALSE;
    }
    ...
    for (rp = relocs; rp < relocs + num_relocs; ++rp) {
      ...
      if (target_specific_reloc_handling(filedata, rp, start, end, symtab,
                                         num_syms))
        continue;
        ...
        reloc_size = 1;
                    ...
                    is_32bit_inplace_sub_reloc(filedata, reloc_type)) ||
               is_32bit_inplace_add_reloc(filedata, reloc_type)) {
        reloc_size = 4;
        reloc_inplace = TRUE;
      } else if ((reloc_subtract =
                      is_64bit_inplace_sub_reloc(filedata, reloc_type)) ||
                 is_64bit_inplace_add_reloc(filedata, reloc_type)) {
        reloc_size = 8;
        reloc_inplace = TRUE;
      } else if ((reloc_subtract =
                      is_16bit_inplace_sub_reloc(filedata, reloc_type)) ||
                 is_16bit_inplace_add_reloc(filedata, reloc_type)) {
        reloc_size = 2;
        reloc_inplace = TRUE;
      } else if ((reloc_subtract =
                      is_8bit_inplace_sub_reloc(filedata, reloc_type)) ||
      ...
      if (sym != symtab && ELF_ST_TYPE(sym->st_info) != STT_COMMON &&
        ...
        warn(_("skipping unexpected symbol type %s in section %s relocation "
               "%ld\n"),
             get_symbol_type(filedata, ELF_ST_TYPE(sym->st_info)),
             printable_section_name(filedata, relsec), (long int)(rp - relocs));
        continue;
      }
      ...
      /* R_XTENSA_32, R_PJ_DATA_DIR32 and R_D30V_32_NORMAL are
         partial_inplace.  */
      if (!is_rela ||
          (filedata->file_header.e_machine == EM_XTENSA && reloc_type == 1) ||
...
#ifdef SUPPORT_DISASSEMBLY
static bfd_boolean disassemble_section(Elf_Internal_Shdr *section,
                                       Filedata *filedata) {
...
static char *get_section_contents(Elf_Internal_Shdr *section,
                                  Filedata *filedata) {
...
uncompress_section_contents(unsigned char **buffer,
  ...
  while (strm.avail_in > 0) {
    if (rc != Z_OK)
      goto fail;
    strm.next_out =
        ((Bytef *)uncompressed_buffer + (uncompressed_size - strm.avail_out));
...
static bfd_boolean dump_section_as_strings(Elf_Internal_Shdr *section,
                                           Filedata *filedata) {
  Elf_Internal_Shdr *relsec;
  ...
  if (start == NULL)
    ...
    return (section->sh_size == 0 || section->sh_type == SHT_NOBITS) ? TRUE
                                                                     : FALSE;
  ...
  if (decompress_dumps) {
    ...
    if ((section->sh_flags & SHF_COMPRESSED) != 0) {
      ...
      unsigned int compression_header_size =
          get_compression_header(&chdr, (unsigned char *)start, num_bytes);
    ...
    if (uncompressed_size) {
      if (uncompress_section_contents(&start, uncompressed_size, &new_size))
        num_bytes = new_size;
      else {
  ...
  for (relsec = filedata->section_headers;
    ...
    printf(_("  Note: This section has relocations against it, but these have "
             "NOT been applied to this dump.\n"));
    break;
  }
    ...
    if (data < end) {
      ...
      if (maxlen > 0) {
        ...
        while (maxlen) {
          ...
          /* Do not print control characters directly as they can affect
             terminal settings.  Such characters usually appear in the names
             generated by the assembler for local labels.  */
          if (ISCNTRL(c)) {
            printf("^%c", c + 0x40);
          } else if (ISPRINT(c)) {
            putchar(c);
...
static bfd_boolean dump_section_as_bytes(Elf_Internal_Shdr *section,
                                         Filedata *filedata,
...
#ifdef ENABLE_LIBCTF
static ctf_sect_t *shdr_to_ctf_sect(ctf_sect_t *buf, Elf_Internal_Shdr *shdr,
                                    Filedata *filedata) {
...
static void dump_ctf_errs(ctf_dict_t *fp) {
  ...
  while ((errtext = ctf_errwarning_next(fp, &it, &is_warning, &err)) != NULL) {
    error(_("%s: %s"), is_warning ? _("warning") : _("error"), errtext);
    free(errtext);
  }
...
static int dump_ctf_archive_member(ctf_dict_t *ctf, const char *name,
                                   void *arg) {
  ctf_dict_t *parent = (ctf_dict_t *)arg;
  const char *things[] = {"Header",           "Labels",    "Data objects",
                          "Function objects", "Variables", "Types",
                          "Strings",          ""};
  const char **thing;
  size_t i;
  int err = 0;
...
static bfd_boolean dump_section_as_ctf(Elf_Internal_Shdr *section,
                                       Filedata *filedata) {
  ...
  if (dump_ctf_symtab_name && dump_ctf_symtab_name[0] != 0) {
    ...
    if ((symdata = (void *)get_data(NULL, filedata, symtab_sec->sh_offset, 1,
                                    symtab_sec->sh_size, _("symbols"))) == NULL)
      goto fail;
  ...
  if (dump_ctf_strtab_name && dump_ctf_strtab_name[0] != 0) {
    ...
    if ((strdata = (void *)get_data(NULL, filedata, strtab_sec->sh_offset, 1,
                                    strtab_sec->sh_size, _("strings"))) == NULL)
      goto fail;
  ...
  if (dump_ctf_parent_name) {
    ...
    if ((parentdata = (void *)get_data(NULL, filedata, parent_sec->sh_offset, 1,
                                       parent_sec->sh_size, _("CTF parent"))) ==
        NULL)
      goto fail;
  ...
  if (parentdata) {
    if ((parenta = ctf_arc_bufopen(&parentsect, symsectp, strsectp, &err)) ==
        NULL) {
      dump_ctf_errs(NULL);
      error(_("CTF open failure: %s\n"), ctf_errmsg(err));
      goto fail;
...
load_specific_debug_section(enum dwarf_section_display_enum debug,
  ...
  section->start = (unsigned char *)get_data(NULL, filedata, sec->sh_offset, 1,
                                             sec->sh_size, buf);
  if (section->start == NULL)
    section->size = 0;
  else {
    ...
    if (uncompressed_size) {
      if (uncompress_section_contents(&start, uncompressed_size, &size)) {
...
unsigned char *get_build_id(void *data) {
  Filedata *filedata = (Filedata *)data;
  ...
  for (i = 0, shdr = filedata->section_headers;
       i < filedata->file_header.e_shnum && shdr != NULL; i++, shdr++) {
...
malformed note encountered in section %s whilst scanning for build-id note\n"),
      ...
      inote.descdata =
          ((char *)enote + ELF_NOTE_DESC_OFFSET(inote.namesz, align));
...
malformed note encountered in section %s whilst scanning for build-id note\n"),
    ...
    if (inote.namesz > 0 && const_strneq(inote.namedata, "GNU") &&
...
bfd_boolean load_debug_section(enum dwarf_section_display_enum debug,
                               void *data) {
  struct dwarf_section *section = &debug_displays[debug].section;
  Elf_Internal_Shdr *sec;
  Filedata *filedata = (Filedata *)data;
  ...
  if (filedata->string_table == NULL &&
    ...
    if (strs != NULL && strs->sh_size != 0) {
      filedata->string_table = (char *)get_data(
          NULL, filedata, strs->sh_offset, 1, strs->sh_size, _("string table"));
  ...
  sec =
      find_section_in_set(filedata, section->uncompressed_name, section_subset);
  if (sec != NULL)
    section->name = section->uncompressed_name;
  else {
    sec =
        find_section_in_set(filedata, section->compressed_name, section_subset);
    if (sec != NULL)
...
static void initialise_dumps_byname(Filedata *filedata) {
...
static unsigned char *display_tag_value(signed int tag, unsigned char *p,
...
static unsigned char *display_arc_attribute(unsigned char *p,
                                            const unsigned char *const end) {
  unsigned int tag;
  unsigned int val;
...
static const char *const arm_attr_tag_CPU_arch[] = {"Pre-v4",
                                                    "v4",
                                                    "v4T",
                                                    "v5T",
                                                    "v5TE",
...
static const char *const arm_attr_tag_THUMB_ISA_use[] = {"No", "Thumb-1",
                                                         "Thumb-2", "Yes"};
static const char *const arm_attr_tag_FP_arch[] = {
    "No",        "VFPv1",        "VFPv2",
    "VFPv3",     "VFPv3-D16",    "VFPv4",
    "VFPv4-D16", "FP for ARMv8", "FPv5/FP-D16 for ARMv8"};
...
static const char *const arm_attr_tag_PCS_config[] = {
    "None",           "Bare platform",       "Linux application",
    "Linux DSO",      "PalmOS 2004",         "PalmOS (reserved)",
    "SymbianOS 2004", "SymbianOS (reserved)"};
static const char *const arm_attr_tag_ABI_PCS_R9_use[] = {"V6", "SB", "TLS",
                                                          "Unused"};
static const char *const arm_attr_tag_ABI_PCS_RW_data[] = {
    "Absolute", "PC-relative", "SB-relative", "None"};
static const char *const arm_attr_tag_ABI_PCS_RO_data[] = {
    "Absolute", "PC-relative", "None"};
static const char *const arm_attr_tag_ABI_PCS_GOT_use[] = {"None", "direct",
                                                           "GOT-indirect"};
static const char *const arm_attr_tag_ABI_PCS_wchar_t[] = {"None", "??? 1", "2",
                                                           "??? 3", "4"};
static const char *const arm_attr_tag_ABI_FP_rounding[] = {"Unused", "Needed"};
static const char *const arm_attr_tag_ABI_FP_denormal[] = {"Unused", "Needed",
                                                           "Sign only"};
static const char *const arm_attr_tag_ABI_FP_exceptions[] = {"Unused",
                                                             "Needed"};
static const char *const arm_attr_tag_ABI_FP_user_exceptions[] = {"Unused",
                                                                  "Needed"};
...
static const char *const arm_attr_tag_FP_HP_extension[] = {"Not Allowed",
                                                           "Allowed"};
static const char *const arm_attr_tag_ABI_FP_16bit_format[] = {
    "None", "IEEE 754", "Alternative Format"};
static const char *const arm_attr_tag_DSP_extension[] = {"Follow architecture",
                                                         "Allowed"};
static const char *const arm_attr_tag_MPextension_use[] = {"Not Allowed",
                                                           "Allowed"};
...
static const char *const arm_attr_tag_MPextension_use_legacy[] = {"Not Allowed",
                                                                  "Allowed"};
...
static unsigned char *display_gnu_attribute(
    unsigned char *p,
    unsigned char *(*display_proc_gnu_attribute)(unsigned char *, unsigned int,
...
display_m68k_gnu_attribute(unsigned char *p, unsigned int tag,
                           const unsigned char *const end) {
  unsigned int val;
...
static unsigned char *
display_power_gnu_attribute(unsigned char *p, unsigned int tag,
                            const unsigned char *const end) {
  unsigned int val;
...
static unsigned char *
display_s390_gnu_attribute(unsigned char *p, unsigned int tag,
                           const unsigned char *const end) {
  unsigned int val;
...
static void display_sparc_hwcaps(unsigned int mask) {
  if (mask) {
    bfd_boolean first = TRUE;
...
static void display_sparc_hwcaps2(unsigned int mask) {
  if (mask) {
    bfd_boolean first = TRUE;
...
static unsigned char *
display_sparc_gnu_attribute(unsigned char *p, unsigned int tag,
                            const unsigned char *const end) {
  unsigned int val;
...
static void print_mips_fp_abi_value(unsigned int val) {
  switch (val) {
...
static unsigned char *
display_mips_gnu_attribute(unsigned char *p, unsigned int tag,
                           const unsigned char *const end) {
...
static void display_raw_attribute(unsigned char *p,
...
static unsigned char *display_msp430_attribute(unsigned char *p,
                                               const unsigned char *const end) {
  unsigned int val;
  unsigned int tag;
...
static unsigned char *
display_msp430_gnu_attribute(unsigned char *p, unsigned int tag,
                             const unsigned char *const end) {
...
static unsigned char *display_riscv_attribute(unsigned char *p,
                                              const unsigned char *const end) {
  unsigned int val;
  unsigned int tag;
...
static unsigned char *display_csky_attribute(unsigned char *p,
                                             const unsigned char *const end) {
  unsigned int tag;
  unsigned int val;
  READ_ULEB(tag, p, end);
...
static bfd_boolean process_attributes(
    Filedata *filedata, const char *public_name, unsigned int proc_type,
  ...
  for (i = 0, sect = filedata->section_headers;
       i < filedata->file_header.e_shnum; i++, sect++) {
      ...
      while (section_len > 0) {
        ...
        while (attr_len > 0 && p < contents + sect->sh_size) {
          ...
          if (public_section && display_pub_attribute != NULL) {
            ...
            while (p < end)
              p = display_gnu_attribute(p, display_proc_gnu_attribute, end);
            assert(p == end);
          } else if (p < end) {
...
static bfd_vma print_mips_got_entry(unsigned char *data, bfd_vma pltgot,
                                    bfd_vma addr, unsigned char *data_end) {
  printf("  ");
...
static bfd_vma print_mips_pltgot_entry(unsigned char *data, bfd_vma pltgot,
                                       bfd_vma addr) {
  printf("  ");
  print_vma(addr, LONG_HEX);
  printf(" ");
...
static void print_mips_ases(unsigned int mask) {
  if (mask & AFL_ASE_DSP)
    fputs("\n\tDSP ASE", stdout);
...
static void print_mips_isa_ext(unsigned int isa_ext) {
  switch (isa_ext) {
  case 0:
    fputs(_("None"), stdout);
    break;
...
static signed int get_mips_reg_size(int reg_size) {
  return (reg_size == AFL_REG_NONE)
             ? 0
             : (reg_size == AFL_REG_32)
                   ? 32
                   : (reg_size == AFL_REG_64)
                         ? 64
                         : (reg_size == AFL_REG_128) ? 128 : -1;
}

static bfd_boolean process_mips_specific(Filedata *filedata) {
  Elf_Internal_Dyn *entry;
  ...
  if (sect != NULL) {
    ...
    if (sizeof(Elf_External_ABIFlags_v0) != sect->sh_size) {
      ...
      abiflags_ext = get_data(NULL, filedata, sect->sh_offset, 1, sect->sh_size,
      ...
      if (abiflags_ext) {
        ...
        printf("\nGPR size: %d", get_mips_reg_size(abiflags_in.gpr_size));
        printf("\nCPR1 size: %d", get_mips_reg_size(abiflags_in.cpr1_size));
        printf("\nCPR2 size: %d", get_mips_reg_size(abiflags_in.cpr2_size));
        fputs("\nFP ABI: ", stdout);
  ...
  if (filedata->dynamic_section == NULL) {
    ...
    if (sect != NULL) {
      ...
      /* In a dynamic binary GOT[0] is reserved for the dynamic
         ...
         before fetching it.  */
      if (data && data + ent - pltgot + addr_size <= data_end &&
        ...
        /* Check for the MSB of GOT[1] being set, identifying a
           ...
           before fetching it.  */
        if (data && data + ent - pltgot + addr_size <= data_end &&
            (byte_get(data + ent - pltgot, addr_size) >> (addr_size * 8 - 1)) !=
                0) {
          ent = print_mips_got_entry(data, pltgot, ent, data_end);
          printf("\n");
          if (ent == (bfd_vma)-1)
  ...
  if (liblist_offset != 0 && liblistno != 0 && do_dynamic) {
    ...
    elib = (Elf32_External_Lib *)get_data(NULL, filedata, liblist_offset,
                                          sizeof(Elf32_External_Lib), liblistno,
    ...
    if (elib) {
      printf(ngettext("\nSection '.liblist' contains %lu entry:\n",
             ...
             (unsigned long)liblistno);
      fputs(_("     Library              Time Stamp          Checksum   "
              "Version Flags\n"),
            stdout);
      ...
      for (cnt = 0; cnt < liblistno; ++cnt) {
        ...
        if (liblist.l_flags == 0)
          ...
          static const struct {
            const char *name;
            int bit;
          } l_flags_vals[] = {{" EXACT_MATCH", LL_EXACT_MATCH},
    ...
    if (eopt) {
      ...
      while (cnt-- > 0) {
        ...
        case ODK_NULL:
          ...
          printf(" NULL       %" PRId16 " %" PRIx32, option.section,
                 option.info);
          break;
        ...
        case ODK_REGINFO:
          ...
          if (filedata->file_header.e_machine == EM_MIPS) {
            ...
            printf("GPR %08" PRIx32 "  GP 0x%" PRIx32 "\n", reginfo.ri_gprmask,
                   reginfo.ri_gp_value);
            printf("          "
                   "  CPR0 %08" PRIx32 "  CPR1 %08" PRIx32 "  CPR2 %08" PRIx32
                   "  CPR3 %08" PRIx32 "\n",
        ...
        case ODK_GP_GROUP:
          printf(" GP_GROUP  %#06x  self-contained %#06x",
                 option.info & OGP_GROUP, (option.info & OGP_SELF) >> 16);
          break;
        ...
        default:
          /* This shouldn't happen.  */
          printf(" %3d ???     %" PRId16 " %" PRIx32, option.kind,
  ...
  if (conflicts_offset != 0 && conflictsno != 0) {
    ...
    if (is_32bit_elf) {
      ...
      econf32 = (Elf32_External_Conflict *)get_data(
          NULL, filedata, conflicts_offset, sizeof(*econf32), conflictsno,
          _("conflict"));
      if (!econf32) {
        free(iconf);
        return FALSE;
      ...
      econf64 = (Elf64_External_Conflict *)get_data(
          NULL, filedata, conflicts_offset, sizeof(*econf64), conflictsno,
          _("conflict"));
      if (!econf64) {
        free(iconf);
        return FALSE;
  ...
  if (pltgot != 0 && local_gotno != 0) {
    ...
    if (symtabno < gotsym) {
      error(_("The GOT symbol offset (%lu) is greater than the symbol table "
              "size (%lu)\n"),
    ...
    data =
        (unsigned char *)get_data(NULL, filedata, offset, global_end - pltgot,
                                  1, _("Global Offset Table data"));
    /* PR 12855: Null data is handled gracefully throughout.  */
    ...
    printf(_("  %*s %10s %*s Purpose\n"), addr_size * 2, _("Address"),
           _("Access"), addr_size * 2, _("Initial"));
    ent = print_mips_got_entry(data, pltgot, ent, data_end);
    ...
    if (data != NULL && gotsym < symtabno) {
      ...
      printf("  %*s %10s %*s %*s %-7s %3s %s\n", addr_size * 2, _("Address"),
             _("Access"), addr_size * 2, _("Initial"), addr_size * 2,
             _("Sym.Val."), _("Type"),
      ...
      for (i = gotsym; i < symtabno; i++) {
        ...
        if (filedata->dynamic_symbols == NULL)
          ...
          if (VALID_DYNAMIC_NAME(filedata, psym->st_name))
            print_symbol(sym_width, GET_DYNAMIC_NAME(filedata, psym->st_name));
          else
            printf(_("<corrupt: %14ld>"), psym->st_name);
        } else
  ...
  if (mips_pltgot != 0 && jmprel != 0 && pltrel != 0 && pltrelsz != 0) {
    ...
    printf("\nPLT GOT:\n\n");
    printf(_(" Reserved entries:\n"));
    printf(_("  %*s %*s Purpose\n"), addr_size * 2, _("Address"), addr_size * 2,
           _("Initial"));
    ...
    printf("  %*s %*s %*s %-7s %3s %s\n", addr_size * 2, _("Address"),
           addr_size * 2, _("Initial"), addr_size * 2, _("Sym.Val."), _("Type"),
           _("Ndx"), _("Name"));
...
static bfd_boolean process_nds32_specific(Filedata *filedata) {
  Elf_Internal_Shdr *sect = NULL;
...
static bfd_boolean process_gnu_liblist(Filedata *filedata) {
  ...
  for (i = 0, section = filedata->section_headers;
    ...
    case SHT_GNU_LIBLIST:
      ...
      elib = (Elf32_External_Lib *)get_data(NULL, filedata, section->sh_offset,
                                            1, section->sh_size,
                                            _("liblist section data"));
      ...
      printf(ngettext("\nLibrary list section '%s' contains %lu entries:\n",
                      "\nLibrary list section '%s' contains %lu entries:\n",
                      num_liblist),
             printable_section_name(filedata, section), num_liblist);

      puts(_("     Library              Time Stamp          Checksum   Version "
             "Flags"));
      ...
      for (cnt = 0; cnt < section->sh_size / sizeof(Elf32_External_Lib);
        ...
        printf("%3lu: ", (unsigned long)cnt);
        if (do_wide)
          printf("%-20s", liblist.l_name < strtab_size ? strtab + liblist.l_name
                                                       : _("<corrupt>"));
        else
...
static const char *get_note_type(Filedata *filedata, unsigned e_type) {
  static char buff[64];
...
static bfd_boolean print_core_note(Elf_Internal_Note *pnote) {
  ...
  printf(_("    %*s%*s%*s\n"), (int)(2 + 2 * addr_size), _("Start"),
         (int)(4 + 2 * addr_size), _("End"), (int)(4 + 2 * addr_size),
         _("Page Offset"));
...
static void decode_x86_compat_isa(unsigned int bitmask) {
  while (bitmask) {
...
static void decode_x86_compat_2_isa(unsigned int bitmask) {
  if (!bitmask) {
    printf(_("<None>"));
    return;
  }
...
static void decode_x86_isa(unsigned int bitmask) {
  while (bitmask) {
    unsigned int bit = bitmask & (-bitmask);
...
static void decode_x86_feature_2(unsigned int bitmask) {
  if (!bitmask) {
    printf(_("<None>"));
    return;
  }
...
static void decode_aarch64_feature_1_and(unsigned int bitmask) {
  while (bitmask) {
    unsigned int bit = bitmask & (-bitmask);
...
static void print_gnu_property_note(Filedata *filedata,
    ...
    if (type >= GNU_PROPERTY_LOPROC && type <= GNU_PROPERTY_HIPROC) {
      if (filedata->file_header.e_machine == EM_X86_64 ||
        ...
        case GNU_PROPERTY_X86_ISA_1_USED:
          if (datasz != 4)
            printf(_("x86 ISA used: <corrupt length: %#x> "), datasz);
          else {
            printf("x86 ISA used: ");
        ...
        case GNU_PROPERTY_X86_ISA_1_NEEDED:
          if (datasz != 4)
            printf(_("x86 ISA needed: <corrupt length: %#x> "), datasz);
          else {
        ...
        case GNU_PROPERTY_X86_FEATURE_1_AND:
          if (datasz != 4)
            printf(_("x86 feature: <corrupt length: %#x> "), datasz);
          else {
            printf("x86 feature: ");
        ...
        case GNU_PROPERTY_X86_FEATURE_2_USED:
          if (datasz != 4)
            printf(_("x86 feature used: <corrupt length: %#x> "), datasz);
          else {
...
static bfd_boolean print_gnu_note(Filedata *filedata,
                                  Elf_Internal_Note *pnote) {
...
static const char *get_v850_elf_note_type(enum v850_notes n_type) {
  static char buff[64];
...
static bfd_boolean print_v850_note(Elf_Internal_Note *pnote) {
  unsigned int val;
...
static bfd_boolean process_netbsd_elf_note(Elf_Internal_Note *pnote) {
...
static const char *get_freebsd_elfcore_note_type(Filedata *filedata,
                                                 unsigned e_type) {
  switch (e_type) {
...
static const char *get_netbsd_elfcore_note_type(Filedata *filedata,
                                                unsigned e_type) {
  static char buff[64];
...
static const char *get_stapsdt_note_type(unsigned e_type) {
  static char buff[64];
...
static bfd_boolean print_stapsdt_note(Elf_Internal_Note *pnote) {
  size_t len, maxlen;
...
static const char *get_ia64_vms_note_type(unsigned e_type) {
  static char buff[64];
...
static bfd_boolean print_ia64_vms_note(Elf_Internal_Note *pnote) {
...
get_symbol_for_build_attribute(Filedata *filedata, unsigned long offset,
                               bfd_boolean is_open_attr, const char **pname) {
  ...
  if (filedata->section_headers != NULL &&
    ...
    for (symsec = filedata->section_headers;
      ...
      if (symsec->sh_type == SHT_SYMTAB &&
          get_symtab(filedata, symsec, &ba_cache.symtab, &ba_cache.nsyms,
...
static bfd_boolean same_section(Filedata *filedata, unsigned long addr1,
                                unsigned long addr2) {
...
static bfd_boolean print_gnu_build_attribute_name(Elf_Internal_Note *pnote) {
  ...
  static const char bool_expected[3] = {GNU_BUILD_ATTRIBUTE_TYPE_BOOL_TRUE,
                                        GNU_BUILD_ATTRIBUTE_TYPE_BOOL_FALSE, 0};
  char name_type;
  char name_attribute;
  ...
  if ((unsigned long)(name - pnote->namedata) > pnote->namesz) {
    error(_("corrupt name field: namesz: %lu but parsing gets to %ld\n"),
          (unsigned long)pnote->namesz, (long)(name - pnote->namedata));
    return FALSE;
  }
...
static bfd_boolean process_note(Elf_Internal_Note *pnote, Filedata *filedata) {
...
static bfd_boolean process_notes_at(Filedata *filedata,
                                    Elf_Internal_Shdr *section, bfd_vma offset,
                                    bfd_vma length, bfd_vma align) {
  ...
  while ((char *)external < end) {
    ...
    if (!is_ia64_vms(filedata)) {
      ...
      inote.descdata =
          ((char *)external + ELF_NOTE_DESC_OFFSET(inote.namesz, align));
    ...
    if ((size_t)(inote.descdata - inote.namedata) < inote.namesz ||
      ...
      warn(_(" type: 0x%lx, namesize: 0x%08lx, descsize: 0x%08lx, alignment: "
             "%u\n"),
...
static bfd_boolean process_corefile_note_segments(Filedata *filedata) {
  ...
  for (i = 0, segment = filedata->program_headers;
    ...
    if (segment->p_type == PT_NOTE)
      if (!process_notes_at(filedata, NULL, (bfd_vma)segment->p_offset,
...
static bfd_boolean process_v850_notes(Filedata *filedata, bfd_vma offset,
                                      bfd_vma length) {
  Elf_External_Note *pnotes;
  Elf_External_Note *external;
  char *end;
...
static bfd_boolean process_note_sections(Filedata *filedata) {
  Elf_Internal_Shdr *section;
  unsigned long i;
  unsigned int n = 0;
  bfd_boolean res = TRUE;
  ...
  for (i = 0, section = filedata->section_headers;
       i < filedata->file_header.e_shnum && section != NULL; i++, section++) {
    if (section->sh_type == SHT_NOTE) {
      if (!process_notes_at(filedata, section, (bfd_vma)section->sh_offset,
    ...
    if ((filedata->file_header.e_machine == EM_V800 ||
      ...
      if (!process_v850_notes(filedata, (bfd_vma)section->sh_offset,
                              (bfd_vma)section->sh_size))
        res = FALSE;
      n++;
    }
...
static bfd_boolean process_notes(Filedata *filedata) {
...
static unsigned char *
display_generic_attribute(unsigned char *start, unsigned int tag,
                          const unsigned char *const end) {
  if (tag == 0)
    return (unsigned char *)end;
...
static bfd_boolean process_arch_specific(Filedata *filedata) {
  ...
  case EM_ARC_COMPACT2:
    return process_attributes(filedata, "ARC", SHT_ARC_ATTRIBUTES,
                              display_arc_attribute, display_generic_attribute);
  case EM_ARM:
    return process_attributes(filedata, "aeabi", SHT_ARM_ATTRIBUTES,
                              display_arm_attribute, display_generic_attribute);
...
static bfd_boolean get_file_header(Filedata *filedata) {
  ...
  if (is_32bit_elf) {
    ...
    if (fread(ehdr32.e_type, sizeof(ehdr32) - EI_NIDENT, 1, filedata->handle) !=
        1)
      return FALSE;
...
64 bit data type and so it cannot read 64 bit ELF files.\n"));
    ...
    if (fread(ehdr64.e_type, sizeof(ehdr64) - EI_NIDENT, 1, filedata->handle) !=
        1)
      return FALSE;
...
static void close_file(Filedata *filedata) {
  if (filedata) {
    if (filedata->handle)
...
void close_debug_file(void *data) { close_file((Filedata *)data); }

static Filedata *open_file(const char *pathname) {
  struct stat statbuf;
...
void *open_debug_file(const char *pathname) { return open_file(pathname); }
...
static bfd_boolean process_archive(Filedata *filedata,
    ...
    if (arch.sym_table == NULL)
      ...
      for (i = l = 0; i < arch.index_num; i++) {
        if (i == 0 ||
          ...
          char *member_name = get_archive_member_name_at(
              &arch, arch.index_array[i], &nested_arch);

          if (member_name != NULL) {
            char *qualified_name =
                make_qualified_name(&arch, &nested_arch, member_name);
  ...
  while (1) {
    ...
    if (is_thin_archive && arch.nested_member_origin == 0) {
      ...
      char *member_file_name =
          adjust_relative_path(filedata->file_name, name, namelen);
      ...
      if (fseek(nested_arch.file, filedata->archive_file_offset, SEEK_SET) !=
          0) {
...
static bfd_boolean process_file(char *file_name) {
  Filedata *filedata = NULL;
  struct stat statbuf;
  char armag[SARMAG];
  bfd_boolean ret = TRUE;
...
void db_task_printsym(unsigned int addr) { print_address(addr, stderr); }
#endif
